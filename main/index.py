...
class InvertedIndexReader(InvertedIndex):
    ...
    def __next__(self):
        ...
        """
        try:
            next_term = next(self.term_iter)
            postings_list, tf_list = self.get_postings_list(next_term)
            return next_term, postings_list, tf_list
    ...
    def get_postings_list(self, term):
        ...
        """
        start_position, no_of_docs, doc_id_length_in_bytes, tf_length_in_bytes = self.postings_dict.get(term, (0,0,0,0))
        self.index_file.seek(start_position)
        encoded_doc_id_postings = self.index_file.read(doc_id_length_in_bytes)
        encoded_tf_postings = self.index_file.read(tf_length_in_bytes)

        decoded_doc_postings = self.postings_encoding.decode(encoded_doc_id_postings)
        decoded_tf_postings = self.postings_encoding.decode_tf(encoded_tf_postings)

        assert len(decoded_doc_postings) == no_of_docs
        assert len(decoded_tf_postings) == no_of_docs
        
        return decoded_doc_postings, decoded_tf_postings
...
class InvertedIndexWriter(InvertedIndex):
    ...
    def append(self, term, postings_list, tf_list):
        ...
        """
        for i,document in enumerate(postings_list):
            if document not in self.doc_length:
                self.doc_length[document] = 0
            self.doc_length[document] += tf_list[i]
        ...
        final_position : int = self.index_file.tell()
        postings_list_posision = final_position
        self.index_file.write(encoded_postings_list)
        self.index_file.write(encoded_tf_list)
        self.postings_dict[term] = (postings_list_posision, len(postings_list), len(encoded_postings_list), len(encoded_tf_list))
